...
//     current_id: usize,
//     pub concrete_cms: HashMap<PolyID, PCS::Commitment>,
//     pub virtual_cms: HashMap<PolyID, Vec<(E::ScalarField, Vec<PolyID>)>>,
//     pub eval_maps: HashMap<PolyID, Box<dyn Fn(PolyId, E::ScalarField) -> E::ScalarField>>,
// }

// impl<E: Pairing, PCS: PolynomialCommitmentScheme<E>> VerifierTracker<E, PCS> {
//     fn add_concrete_cm(&mut self, cm_s: &HashMap<PolyID, PCS::Commitment>, f: impl Fn(PolyId, E::ScalarField) -> E::ScalarField) -> PolyID {
//         self.current_id += 1;
//         let id = PolyID(current_id.to_string());
//         self.concrete_cms.insert(id.clone(), cm_s[&id].clone());
...
//     // Prover expects a commitment from the verifier
//     // generate a polynomial p
//     let (id, cm) = tracker.add_mat_poly(p); // This guy will update the internal HashMap (materialized_polys and materialized_cms)
...
// // Proof {
// //     pub commitments: HashMap<PolyID, PCS::Commitment>,
// //     pub evaluations: HashMap<(PolyID, E::ScalarrField), E::ScalarField>,
